...
過去のCSV/NPZから廃止したUCB結果を除外し、バージョン由来のモード名と旧凡例表記
（`AGG_INTERVAL`、`batch`、`δ_adwin`、FedDriftの`δ`）を現在表記へ変換する場合は、
元ディレクトリとは別の出力先を指定する。
...
元データは変更しない。CSV/NPZだけを移行し、PNGやMarkdownなどの派生成果物はコピーしないため、
移行後の一次結果からPareto図・回復曲線・集計表を再生成する。処理件数と除外対象は
`migration_manifest.json`へ記録される。
