...
python run_experiment.py --mode FedSDA --seed 0 --dataset sea --concept-schedule feddrift_fixed
...
| `FedSDA_v2.1` | **提案手法 v2.1**: v2の通信フローを維持し、全体損失ADWINと正解クラス別損失ADWINを並列監視。局所的・クラス不均衡なドリフトの検出を補強 |
| `FedSDA_v3.1` | **提案手法 v3.1**: v3のキャッシュ評価・通信フローに、v2.1と同じ全体損失ADWIN＋正解クラス別損失ADWINを適用 |
| `FedSDA_v2.2` / `FedSDA_v3.2` | v2/v3のサーバフローに、bounded mean向け混合Shiryaev–Roberts型e-detectorを適用。`E_DETECTOR_ALPHA`は誤検知確率ではなく平均誤警報間隔(ARL)を制御 |
| `FedSDA_v2.3` / `FedSDA_v3.3` | v2/v3のサーバフローに、全体損失e-SRと正解クラス別e-SRの固定重み混合を適用 |
| `FedSDA_v2.2_ucb` / `v2.3_ucb` / `v3.2_ucb` / `v3.3_ucb` | 対応するe-SRモードの基準平均をempirical Bernstein上側信頼限界へ置換するアブレーション |
