...
python run_experiment.py --mode FedSDA_NoCached_ADWIN --seed 0 --dataset sea4

# 同じデータ分布でFedDrift論文の固定ドリフト系列を使用
python run_experiment.py --mode FedSDA_NoCached_ADWIN --seed 0 --dataset sea4 --concept-schedule feddrift_fixed
...
# 精度–通信量の掃引(FedSDA δ_ADWIN / FedDrift B_detect・δ_FedDrift)
python run_pareto_sweep.py --datasets sea4 circle2 sine2 --seeds 0

# プロトコルと検出器を限定して掃引
python run_pareto_sweep.py --datasets sea4 --seeds 0 1 2 `
  --fedsda-modes FedSDA_NoCached_ADWIN FedSDA_Cached_ADWIN --feddrift-modes FedDrift

# 掃引をシードごとに分割して積み増した後、複数CSVをシード平均で1枚に再描画
python run_pareto_sweep.py --plot-csvs "results/pareto/pareto_sea4-circle2-sine2_seed*_n5000.csv"

# 回復曲線(適応速度)分析: 生データを保存 → acc(Δ) を集計・描画
python run_pareto_sweep.py --datasets sine2 --seeds 0 1 2 --raw-dir results/raw
python recovery_analysis.py --npz "results/raw/*sine2*.npz"
...
| `FedSDA_NoCached_ResidualAdapter_ClassESR` | 概念別低ランク残差adapterをhard routingで使う対照方式 |
...
FedSDAでは `--clustering-policy on_new_model`（新規モデル発生時のみ、既定）と
...
| `sea4` | 4 | **FedDrift SEA-4**。x1,x2,x3 ~ U[0,10](x3 ノイズ)、label = 1 iff (x1+x2) ≤ 閾値 |
| `sea2` | 2 | **FedDrift SEA-2**。SEAの概念0/1 |
| `circle2` | 2 | **FedDrift CIRCLE-2**。x1,x2 ~ U[0,1]²、概念別の円の外側を label=1(小さな概念変化) |
| `sine2` | 2 | **FedDrift SINE-2**。x1,x2 ~ U[0,1]²、概念0: x2≤sin(x1) を label=1、概念1: 反転(大きな概念変化) |
