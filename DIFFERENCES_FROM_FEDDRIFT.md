...
| 検出粒度 | 時刻（500 サンプル）単位で検出 | FedSDA: **per-sample 逐次**（ADWIN）／FedDrift: 論文の時刻単位検出(500)を独立パラメータ化した `FEDDRIFT_DETECT_BATCH`(既定 50)。FedDrift はこの完了時に集約するため通信間隔も兼ねる（§3 参照） | `clients/`, `experiment.py:72` |
...
|---|---|---|---|---|
| K | 1 ラウンドあたりのローカル学習ステップ数（全手法共通） | 50 | `K_STEPS=50` | ✅ |
...
| L | 1 サンプルあたりのローカル更新回数 | （明記なし） | `UPDATES_PER_STEP=1` | — |
| （検出/時刻粒度） | 1 時刻あたりのデータ数＝FedDrift の検出単位 | 500 | `FEDDRIFT_DETECT_BATCH=50` | ⚠️ |
| R | 1 時刻あたりの通信ラウンド数 | 100 | **直接対応する量なし**（下記） | ⚠️ |

### 論文 K / R と本実装の対応（重要）

論文では 500 サンプル/時刻という 1 単位が「データ到着・ドリフト検出・R ラウンド学習」を
**兼ねて**いる。本実装はストリームを**単一パス**するため、これらを別パラメータに分解している。
そのため**同じ役割でも手法によって担当パラメータが異なる**点に注意。

- **ローカル学習ステップ数（論文 K=50）＝ `K_STEPS`**（両手法共通）。FedDrift は
  `local_train(k_steps=K_STEPS)`（`feddrift.py:102`）、FedSDA は 1 更新/サンプル × K_STEPS。
- **集約（通信）間隔は手法で異なる**:
  - **FedSDA / Oblivious**: `K_STEPS` サンプルごとに集約（`experiment.py:52`）→ 実験全体で
    TOTAL_DATA_POINTS / K_STEPS ≈ 100 回。
  - **FedDrift**: **検出バッチ完了時のみ集約**（`experiment.py:72` は `fired` で gate）→
    集約間隔 ＝ `FEDDRIFT_DETECT_BATCH`。掃引でこれを大きくすると通信が反比例して減る
    （精度–通信量トレードオフの軸）。
- **FedDrift の検出粒度 ＝ `FEDDRIFT_DETECT_BATCH`**。元 FedDrift は**時刻(500)単位で検出**する
  ので、これは**その「検出バッチ」を独立パラメータ化したもの＝概念としては論文の時刻粒度
  そのもの**（既定 50 で値のみ相違）。FedDrift は「バッチ完了時に通信」する設計なので、これが
  集約間隔も兼ねる。
- **1 サンプルあたりの学習回数 ＝ `UPDATES_PER_STEP`（論文 L）**。既定 1
  （1 サンプルにつき 1 ミニバッチ更新／保持モデル。`base.py:116`）。

**論文 R（=100 反復ラウンド）**は「固定 500 サンプルを収束まで反復学習」する量で、上の
「集約間隔」と「学習深さ (L)」の 2 役を兼ねる。本実装は単一パスのためこの 2 役が上記に分かれる。
`R_ROUNDS`（既定 1）は**論文 R とは別概念**で、ストリームを「時刻」に区切る粒度
（`data_per_time = R_ROUNDS × K_STEPS`, `experiment.py:316`）を決めるだけ。総集約回数・
総学習回数は `R_ROUNDS` に依らず一定なので学習・通信・精度に実質影響せず、変わるのは
`paper_accuracy` の評価間隔のみ（＝事実上、汎用化のための残置ノブ）。したがって「通信を
100→1 に減らした」わけではない。
