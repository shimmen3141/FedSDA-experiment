...
> 詳細は [ALGORITHM.md](ALGORITHM.md)、全変数の意味・手法間の使い分けは
...
|---|---|---|---|---|
| K | 1 ラウンドあたりのローカル学習ステップ数 | 50 | `FEDDRIFT_LOCAL_STEPS=50` | ✅ |
...
| （検出/時刻粒度） | 1 時刻あたりのデータ数＝検出単位 | 500 | `FEDDRIFT_DETECT_BATCH=50` | ⚠️ |
| R | 1 時刻あたりの通信ラウンド数 | 100 | `FEDDRIFT_ROUNDS=1`（既定。下記） | ⚠️ |

### 補足: 検出粒度と R の扱い

- **K ＝ `FEDDRIFT_LOCAL_STEPS`**（論文どおり 50）。リプレイバッファからのバッチ学習を 1 ラウンドにつき K 回行う。
- **検出粒度 ＝ `FEDDRIFT_DETECT_BATCH`**。元 FedDrift は**時刻(500 サンプル)単位で検出**する。これを独立パラメータ化し既定 50 とした（概念は論文の時刻粒度そのもの、値のみ相違）。FedDrift はこのバッチ完了時にのみ集約するため、これが**集約(通信)間隔も兼ねる**（掃引で大きくすると通信量が反比例して減る＝精度–通信量トレードオフの軸）。
- **R ＝ `FEDDRIFT_ROUNDS`**。論文はバッチ(500 サンプル)を R=100 ラウンド収束まで反復学習する。本実装は**既定 R=1**。これは、比較対象の提案手法 FedSDA が**オンライン単一パス**（各サンプル 1 回）であり、その学習・通信予算（1 モデルあたり総更新数 =`TOTAL_DATA_POINTS × UPDATES_PER_STEP`）と揃えるため。R=1 なら FedDrift も同じ総更新数になる。`FEDDRIFT_ROUNDS=100` にすれば論文忠実なFedDrift（バッチ収束学習）を再現できるが、総更新数・総通信量が R 倍になり FedSDA との予算一致は崩れる。R は `FEDDRIFT_DETECT_BATCH`（検出粒度↔通信）と**直交する第2の通信軸**（バッチあたり収束度↔通信）。
...
- **サーバ側モデル間距離の評価**: FedDrift 公開実装は aggregator が全クライアントの生データ（`all_data`）を中央に集めて距離を計算する**シミュレーション近道**を取る。本実装はこれに倣わず、評価対象モデルをデータ保持クライアントへ配布して現地評価させ、**集約統計量 (n, Σℓ, Σℓ²) のみ**を返す federated な形にしている（サブサンプリング `CROSS_EVAL_MAX_CLIENTS` / `EVAL_MAX_SAMPLES` による近似）。詳細と根拠は [ALGORITHM.md](ALGORITHM.md) §3 を参照。
