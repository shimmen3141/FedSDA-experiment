...
|---|---|---|---|---|
| K | 集約間隔（何サンプルごとに集約するか） | 50 | `K_STEPS=50` | ✅ |
...
論文の R=100 は「各時刻の**固定 500 サンプル**を K=50 ローカルステップ × 100 ラウンド
反復学習して収束させる」量で、**(a) 通信（集約）頻度** と **(b) バッチあたりの学習深さ**
の 2 役を兼ねる。本実装はストリームを**単一パス**するため、この 2 役は別パラメータに分かれる:

- **(a) 通信（集約）頻度 = `K_STEPS`**。`K_STEPS` サンプルごとにサーバ集約するので、
  実験全体では TOTAL_DATA_POINTS / K_STEPS ≈ 100 回の集約が実際に走る
  （`experiment.py:52` で `K_STEPS` ブロックごとに `server.run_round`）。
- **(b) 1 サンプルあたりの学習回数 = `UPDATES_PER_STEP`（論文の L）**。既定 1
  （1 サンプルにつき 1 ミニバッチ更新／保持モデル。`base.py:116` `train_all_held_models`）。

`R_ROUNDS`（既定 1）は**論文 R とは別概念**で、ストリームを「時刻」に区切る粒度
（`data_per_time = R_ROUNDS × K_STEPS`, `experiment.py:316`）を決めるだけ。総集約回数・
総学習回数は `TOTAL_DATA_POINTS / K_STEPS` で `R_ROUNDS` に依らず一定なので、学習・通信・
精度には実質影響せず、変わるのは `paper_accuracy` の評価間隔のみ（＝事実上、汎用化のための
残置ノブ）。したがって「通信を 100→1 に減らした」わけではなく、通信頻度は `K_STEPS` が
決めている。
