...
# FedDrift の1検出バッチあたりの通信ラウンド数(論文 R に対応)。バッチ完了時に
# {ローカル学習 FEDDRIFT_LOCAL_STEPS → 集約} を R 回繰り返す。
# 既定 1 は FedSDA(オンライン単一パス)と学習・通信予算を揃えた公平比較用。R=100 にすると
# 論文忠実な FedDrift(固定バッチを収束まで反復学習)を再現できるが、総ローカル更新数も
# 総集約(通信)回数も R 倍になり FedSDA と予算が揃わなくなる点に注意。
# FEDDRIFT_DETECT_BATCH(検出粒度↔通信)とは直交する軸(バッチあたり収束度↔通信)。
# FedSDA には対応物なし(逐次のため)。詳細は DIFFERENCES_FROM_FEDDRIFT.md §3。
FEDDRIFT_ROUNDS = 1
