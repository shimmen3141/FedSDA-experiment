...
各変数の意味・使用手法・設計上の役割は HYPERPARAMETERS.md に一覧化している。
...
AGG_INTERVAL = 50           # FedSDA/Oblivious: 1ラウンドで処理するサンプル数(=集約間隔)。FedDrift は FEDDRIFT_DETECT_BATCH
...
# 各データセット(SEA-4/CIRCLE-2/SINE-2)の生成規則は data.py と DIFFERENCES_FROM_FEDDRIFT.md §1 を参照。
...
# FedDrift ベースライン
# ==========================================
# 検出バッチサイズ=FedDrift の1ラウンドで処理するサンプル数(検出粒度・集約間隔・学習量を兼ねる)。詳細は HYPERPARAMETERS.md §7
