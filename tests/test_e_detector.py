...
def test_meta_router_can_mix_global_context_and_leader_without_extra_forward(
    monkeypatch,
):
    monkeypatch.setattr(config, "SOFT_ROUTING_CONTEXT", "meta_predicted_class")
    monkeypatch.setattr(config, "SOFT_ROUTING_META_LOSS", "zero_one")
    monkeypatch.setattr(
        config, "SOFT_ROUTING_META_CANDIDATES", "global_context_leader"
    )
    spec = MODE_SPECS[
        "FedSDA_NoCached_ClassESR_RestartingSoftRouting"
    ]
    ...
    assert client.shadow_meta_routers[0].cumulative_losses == {
        "global_mixture": pytest.approx(1.0),
        "context_mixture": pytest.approx(0.0),
        "context_leader": pytest.approx(0.0),
    }
