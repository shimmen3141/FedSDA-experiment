...
MODE_SPECS = {
    ...
    'FedSDA_NoCached_ResidualAdapter_ADWIN_RestartingSoftRouting': ModeSpec(
        ResidualAdapterADWINRestartingSoftRoutingFedSDAClient,
        _run_per_sample_timestep,
        server_cls=SharedBackboneFedSDANoCachedServer,
        model_cls=ResidualAdapterMLP,
    ...
    'FedSDA_NoCached_ResidualAdapter_ESR_RestartingSoftRouting': ModeSpec(
        ResidualAdapterESRRestartingSoftRoutingFedSDAClient,
        _run_per_sample_timestep,
        server_cls=SharedBackboneFedSDANoCachedServer,
        model_cls=ResidualAdapterMLP,
...
def _add_model_diagnostic_results(
    ...
    results.update({
        ...
        "routing_aggregation_recalibration_check_count": sum(
            getattr(
                client.expert_router,
                "aggregation_recalibration_check_count",
                0,
            )
            for client in clients
            if hasattr(client, "expert_router")
        ),
        "routing_aggregation_recalibration_skip_count": sum(
            getattr(
                client.expert_router,
                "aggregation_recalibration_skip_count",
                0,
            )
            for client in clients
            if hasattr(client, "expert_router")
...
def _save_raw_run(
    ...
    if clients and all(
        ...
        telemetry_arrays[
            "routing_aggregation_recalibration_check_counts"
        ] = np.asarray(
            [
                client.expert_router.aggregation_recalibration_check_count
                for client in clients
            ],
            dtype=np.int32,
        )
        telemetry_arrays[
            "routing_aggregation_recalibration_skip_counts"
        ] = np.asarray(
            [
                client.expert_router.aggregation_recalibration_skip_count
                for client in clients
            ],
            dtype=np.int32,
        )
