...
# 共有表現モデルの定期配布方式。versioned_cacheは、最後に配布した版と比較して
# 変更された共有部・概念別部だけを再送する。
SHARED_MODEL_DISTRIBUTION = "full"
SHARED_MODEL_DISTRIBUTION_CHOICES = ("full", "versioned_cache")
...
# ==========================================
AGGREGATION_INTERVAL = 50   # 1ラウンドで処理するサンプル数(=集約間隔)。FedSDA と Oblivious で共有
# ローカル更新間隔 τ(論文の「t mod τ = 0」)。τ サンプルごとに τ×UPDATES_PER_SAMPLE 回まとめて
# 更新する(総更新回数は不変)。1 = 毎サンプル更新。
LOCAL_UPDATE_INTERVAL = 1
...
# ==========================================
FEDSDA_DISTANCE_THRESHOLD = 0.1     # FedSDA のモデル適合・マージ判定距離閾値
