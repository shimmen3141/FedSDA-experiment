...
- FedSDA Cached/NoCached: 検出器パラメータと AGGREGATION_INTERVAL の掃引
...
FEDDRIFT_DISTANCE_THRESHOLD = parameter("feddrift_distance_threshold").csv_name
ADWIN_DELTA = parameter("adwin_delta").csv_name
...
def _run_resolved(mode, dataset, seed, series, sweep_value, sweep_parameter=None,
    ...
    if (
        is_shared_representation_mode(mode)
        and config.SHARED_MODEL_DISTRIBUTION != "full"
    ):
        display_series = (
            f"{display_series} [distribution="
            f"{config.SHARED_MODEL_DISTRIBUTION}]"
        )
    if (
        "FedSDA" in mode
        and config.FEDSDA_CLUSTERING_CONSOLIDATION != "merge"
    ):
        display_series = (
            f"{display_series} [cluster-consolidation="
            f"{config.FEDSDA_CLUSTERING_CONSOLIDATION}]"
        )
    ...
    row = {
        ...
        "seed": seed, "series": display_series,
        "sweep_parameter": sweep_parameter, "sweep_value": sweep_value,
        FEDDRIFT_DETECTION_BATCH_SIZE: (
            config.FEDDRIFT_DETECTION_BATCH_SIZE if mode in FEDDRIFT_MODES else None
        ),
        ...
        FEDDRIFT_DISTANCE_THRESHOLD: (
            distance_threshold if mode in FEDDRIFT_MODES else None
        ),
        ADWIN_DELTA: config.ADWIN_DELTA if is_adwin_mode(mode) else None,
    }
...
def run_sweep(datasets, seeds, batches, deltas, adwin_deltas, fixed_delta, fixed_batch, fixed_gamma,
    ...
    default_agg = config.AGGREGATION_INTERVAL
    ...
    with open(path, encoding="utf-8") as f:
        ...
        for r in reader:
            ...
            row[FEDDRIFT_DISTANCE_THRESHOLD] = (
                ...
                and feddrift_distance not in (None, "", "None") else float("nan")
            )
            row[FEDSDA_DISTANCE_THRESHOLD] = (
                float(fedsda_distance)
            ...
            row.setdefault("shared_model_distribution", "full")
            ...
            for k in [ADWIN_DELTA] + METRIC_KEYS:
                v = row.get(k)
...
def _series_style(series):
    ...
    method_colors = {
        ...
        "FedSDA_NoCached_ResidualAdapter_SharedClassifier_ClassESR_RestartingSoftRouting": "darkmagenta",
        ...
        for baseline in BASELINE_MODES:
            ...
            if baseline == "Oblivious":
                parameter = _fixed_parameter_label(
                    baseline_rows, AGGREGATION_INTERVAL, "A")
            else:
                parameter = _fixed_parameter_label(
...
""",
    ...
    fedsda.add_argument(
        "--shared-model-distribution",
        choices=config.SHARED_MODEL_DISTRIBUTION_CHOICES,
        default=config.SHARED_MODEL_DISTRIBUTION,
        help=(
            "共有表現モデルの配布方式 "
    ...
    fedsda.add_argument(
        ...
        choices=config.FEDSDA_CLUSTERING_CONSOLIDATIONS,
...
def main(argv=None):
    ...
    selections = {
        ...
        "shared_model_distribution": args.shared_model_distribution,
    ...
    fixed_gamma = args.fixed_gamma if args.fixed_gamma is not None else config.FEDSDA_DISTANCE_THRESHOLD
