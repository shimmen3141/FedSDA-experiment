...
| 変数 | 意味 | 使用 | 既定 |
|---|---|---|---|
| `DATASET` | 使用データ `blobs` / `sea2` / `sea4` / `circle2` / `sine2` / `mnist2` / `mnist4` | 共通 | `blobs` |
| `CONCEPT_SCHEDULE` | 概念切替方式 `random` / `feddrift_fixed` | 共通 | `random` |
| `SEA_THRESHOLDS` | SEA-4 各概念の閾値 θ(論文 appendix 準拠 {9,8,7,9.5}) | sea4 / sea2 | `{0:9,1:8,2:7,3:9.5}` |
| `SEA_LABEL_NOISE` | SEA の内在ラベルノイズ率(標準 10%) | sea4 / sea2 | 0.10 |
| `CIRCLE_PARAMS` | CIRCLE-2 各概念の円 (cx,cy,r) | circle2 のみ | `{0:(0.2,0.5,0.15),1:(0.6,0.5,0.25)}` |
...
| `HDDM_DRIFT_CONFIDENCE` | HDDM-A/Wの各検出系列のドリフト境界に使うconfidence。小さいほど保守的 | HDDM系FedSDA | 0.001 |
...
`ClassHDDMA`は全体と正解クラス別系列へ同じconfidenceでHDDM-Aを適用し、追加の
ハイパーパラメータは持たない。Bonferroni型の補正が必要な場合は、利用者が系列数を考慮して
`HDDM_DRIFT_CONFIDENCE`と警告confidenceを設定する。
...
CLIでは `--clustering-policy` で指定する。Cached方式はクライアント側の配布済みモデルキャッシュを
...
| `functional` | 同一標本上で各モデルだけが正解した割合を双方向に集約し、その大きい方がγ以下なら機能的に重複した統合候補とする。異なる標本を正解する相補的モデルは保持する |
| `class_functional_confidence` | `functional`の固有正解率を正解クラスごとに計算し、左右方向・全クラスへBonferroni補正した片側Wilson信頼下限の最大値がγ以下の場合だけ統合候補とする。一部クラスだけで生じる概念差を保護しつつ、少数クラスの偶発的な1件の不一致だけで統合を拒否することを防ぐ |
...
`functional`はクロス評価ですでに記録しているモデル対の正誤表を使うため、モデル通信やforwardを
追加しない。ここでγは損失増加ではなく「片方のモデルにしか正解できない標本割合」の許容上限を表す。
`connected`では重複関係が推移的でない場合にも連結成分全体が統合され得るため、最初の比較では
全モデル対の条件を要求する`complete` linkageを用いる。

`class_functional_confidence`も同じforwardの正誤表を正解クラス別に集計するため、追加するのは軽量統計量だけで
ある。クラス別標本数が`CLUSTER_MIN_EVAL_N`未満のクラスは判定から除外し、利用可能なクラスがない
場合は全体の固有正解率に同じ信頼下限を適用する。信頼水準には既存の
`FEDSDA_CLUSTERING_CONFIDENCE`を用いる。クラス分布の統計をサーバへ返すため、実環境では
secure aggregationなどによる保護を別途検討する必要がある。
rawには`cross_evaluation_class_*`としてラウンド・クライアント・モデル対・正解クラス別の正誤表を
保存し、全体平均では見えない統合判断を事後診断できるようにする。

`confidence`は標本数が増えるほど非常に小さな差も検出して統合を止めるため、モデル数・通信量が
増える場合がある。`confidence_margin`はγを「無視できる実用的差」として残し、
`(平均損失増加 - γ) / 標準誤差`を判定する。これにより、固定γだけの判定とゼロ差検定の中間を
追加ハイパーパラメータなしで比較できる。
...
`FEDSDA_DETECTION_EPISODES_ENABLED`（既定 `False`）を有効にすると、最初の検出から
`FIFO_BUFFER_SIZE` (`N_FIFO`) サンプルを一つの検出エピソードとして扱う。同一エピソードでは
モデル再利用または新規作成を最大1回だけ実行し、後続の検出は証拠として記録するがモデルを再操作しない。
エピソード長には既存の `N_FIFO` を使うため、新しい長さパラメータは持たない。CLIでは
`--detection-episodes` / `--no-detection-episodes` で切り替える。

### 検証付き仮モデル

`--new-model-creation-policy validated` では、既存モデルの再利用条件を満たさなかった場合でも、
新規モデルを直ちに登録しない。検知区間を時系列順に学習部分と末尾の検証部分へ分け、学習部分だけで
仮モデルを学習する。その後、検証部分における仮モデルと最良の既存モデルの損失を比較し、区間全体と
直近半分の双方で `NEW_MODEL_EARLY_STOPPING_MIN_DELTA` 以上改善した場合だけ登録・切替を行う。
棄却時は現行モデルを維持し、イベントを `create_rejected` として記録する。検出エピソードとは独立して
切り替えられる。

CLIでは `--fifo-size` と `--new-model-validation-fraction` を実験単位で指定できる。例えば
`--fifo-size 50 --new-model-validation-fraction 0.4` は、区間長が50なら先頭30件を学習、末尾20件を
...
指標として出力する。
raw NPZには各提案について位置、検出器、採否、理由、学習／検証件数、比較対象モデルID、区間全体と
直近半分の両モデル損失を保存する。損失差は `既存モデル損失 - 仮モデル損失` と定義され、正の値ほど
仮モデルが優れる。

診断用に、検出警報とモデル操作を `AdaptationEvent` として分離して記録する。従来の切替precision・recallに
加え、未対応切替を `switch_fp_early`、`switch_fp_late`、`switch_fp_duplicate`、
`switch_fp_isolated` に分類する。早期は真の変化前の許容遅延以内、遅延は許容窓終了後の同じ長さ以内、
重複は既に別の切替と対応した許容窓内、それ以外を単独FPとする。
...
モデル増加の原因を追跡するため、同じraw NPZへ次のlineage診断も保存する。この診断は
クラスタリング結果を変更せず、既存実験の精度・通信量には影響しない。

- `model_registration_*`: グローバルモデルID、登録ラウンド、作成クライアント、
  終端時に残存したか、予測時に選択されていた延べサンプル数
- `clustering_*`: クラスタリング時のラウンドとモデルID、最近傍モデルID・距離、
  代表モデルID、クラスタサイズ、クラスタ内最大距離、マージへ参加したか、
  代表モデルへ吸収されたか
- `clustering_evaluated_pair_counts` / `clustering_possible_pair_counts`:
  クラスタ内で距離を計算できたモデル対数と全モデル対数。評価データ不足で距離が
  未定義の場合を、距離が大きい場合と区別する

単独クラスタの最大距離、および評価可能な相手がないモデルの最近傍距離は `NaN`、
