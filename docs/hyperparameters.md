...
| `class_functional` | `functional`の固有正解率を正解クラスごとに計算し、十分な標本があるクラスの最大値がγ以下の場合だけ統合候補とする。一部クラスだけで生じる概念差が全体平均で希釈されることを防ぐ |
...
`class_functional`も同じforwardの正誤表を正解クラス別に集計するため、追加するのは軽量統計量だけで
ある。クラス別標本数が`CLUSTER_MIN_EVAL_N`未満のクラスは判定から除外し、利用可能なクラスがない
場合だけ全体の`functional`へフォールバックする。クラス分布の統計をサーバへ返すため、実環境では
