...
`FEDSDA_MERGE_VALIDATION=candidate_loss`を指定すると、クラスタリングが提示した統合候補について
加重平均パラメータの仮統合モデルを作る。各構成モデルの保持データ上で、仮統合モデルと対応する
構成モデルを同じサンプルに評価し、`仮統合損失−構成モデル損失`の全体平均が0以下の場合だけ
統合を採用する。CLIは`--merge-validation candidate_loss`で、既定の`none`は従来動作を維持する。

候補生成に使う`--clustering-decision`とは独立している。検証時には仮統合モデル本体の追加送信と
軽量統計の返送が1フェーズ増えるが、構成モデルは直前のクロス評価またはCached配布で保持済みとする。
CSVには提案・採用・棄却数、評価サンプル数を記録する。
