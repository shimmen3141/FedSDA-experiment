...
基準平均計算は`EDetectorBaselineEstimator`として検出器本体から分離し、次の2戦略を実装する。

- `HistoricalMeanBaseline`: 標本平均を使う従来方式
- `EmpiricalBernsteinUCB`: `[0,1]`有界損失の標本数・分散・`E_DETECTOR_BASELINE_BETA`から上側信頼限界を作る

UCB方式はモード名末尾の`_UCB`で選び、対応するmean方式を変更しない。全体e-SRではモデル全体の
統計、クラス条件付きe-SRではモデル・正解クラス別の統計を使う。較正標本が2件未満なら上限を
ほぼ1として、根拠のない小さな基準による発火を避ける。

ただし、このUCBは固定された較正標本の平均に対する上限である。モデルのオンライン更新や将来の
データ分布変化を含めて、すべての時点で条件付き平均が上限以下になることまでは保証しない。
従って、mean方式より帰無仮説の設定を保守化するアブレーションとして扱い、厳密な無条件ARL保証と
同一視しない。
...
mean方式は既存結果との後方互換のため、クラス別e-SRにも全体モデル統計から得た基準平均を使う。
UCB方式は新たに保持するクラス別統計からクラス固有の基準を作る。ARL保証をクラス別成分まで厳密に
...
各ESRモードには、同じ検出系列・サーバ処理のまま基準平均だけをUCB化した`*_UCB`モードがある。
