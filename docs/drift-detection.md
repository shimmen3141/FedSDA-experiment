...
- 実装: `federated_drift_experiment/drift_detectors/adwin.py`の`FullScanADWIN`
...
- 実装: `federated_drift_experiment/drift_detectors/e_detector.py`の`BoundedMeanEDetector`
...
- 実装: `federated_drift_experiment/drift_detectors/hddm.py`の`HDDMA` / `HDDMW`
...
FedSDAでは、検出器の警報と、その後に実行されるモデル維持・既存モデル再利用・新規モデル作成を分けて
記録する。`AdaptationEvent` は位置、検出器、操作種別、変更前後のモデルID、推定変化点、検出エピソードIDを
保持する。これにより、検出器自体の `alarm_precision` / `alarm_recall` と、従来の実モデル切替に対する
`precision` / `recall` を混同せず比較できる。

検出エピソードを有効にした場合、`N_FIFO` 幅に入る近接検出を同じエピソードへ統合する。最初のモデル操作後の
追加警報は `episode_suppressed` として記録し、現在モデルの学習データへ反映するが、再切替や連続した
新規モデル作成は行わない。

適応イベントの `create_rejected` は、検出区間から学習した仮モデルが末尾の時系列検証区間で最良の
既存モデルを一貫して上回らず、新規モデル作成を棄却したことを表す。この判定は検出エピソードとは
独立しており、`--new-model-creation-policy validated` で有効になる。

検証付き仮モデルの診断では、棄却理由を `insufficient_data`、`full_interval`、
`recent_interval`、`full_and_recent` に分ける。raw NPZの `provisional_*` 配列を使うと、真のドリフト位置と
提案位置を対応させ、どの損失条件が採否を決めたかを提案単位で再分析できる。
