...
**τ(`LOCAL_UPDATE_INTERVAL`)はこれと直交する config ノブ**で、**モードに依らず全逐次手法
...
circle2のように変化が一部クラス・局所領域へ集中し、全体平均損失では薄まるドリフトを対象とする。
...
**バッチベース**で、`FEDDRIFT_DETECTION_BATCH_SIZE` 件を溜めてから検出・通信する。検出バッチ完了時は
...
sequenceDiagram
    ...
    loop 検出バッチごと（ストリーム全体を通じて繰り返し）
        ...
        loop FEDDRIFT_DETECTION_BATCH_SIZE 回（サンプルをバッファに蓄積）
            D->>C: データ (x_c^t, y_c^t)
        ...
        loop FEDDRIFT_ROUNDS 回（論文 R・既定 1）
            Note over C: リプレイバッファからローカル学習<br/>（FEDDRIFT_DETECTION_BATCH_SIZE × UPDATES_PER_SAMPLE ステップ ＝ 1バッチ分の予算）
            C->>S: 保持モデルのパラメータ(FedAvg 用)
            Note over S: モデルIDごとにデータ量で加重平均(FedAvg)
            S->>C: 全グローバルモデルをブロードキャスト
        end
...
sequenceDiagram
    ...
    loop 検出バッチごと（論文の時刻 τ に対応）
        ...
        loop FEDDRIFT_DETECTION_BATCH_SIZE 回
            D->>C: データ (x_c^t, y_c^t)
            Note over C: 現行モデルで予測を記録<br/>検出バッファに蓄積
        end
...
`disabled` はクラスタリングを行わず、`on_new_model`（既定）は新規モデルの初回配布後だけ、`every_round` はモデルが2個以上ある各集約ラウンドで
