...
### FedSDA v2.2（モード `FedSDA_v2.2`）

v2.2はv2の通信・サーバ処理を維持し、全体損失ADWINをbounded mean向け混合e-SR検知器へ
...
サーバ通信の矢印はv2と完全に同じで、図中の「ADWINへ投入(+強制チェック)」だけを
「全体損失e-SRを更新」に読み替える。

### FedSDA v2.3（モード `FedSDA_v2.3`）

v2.3はv2.2の全体損失e-SRに、正解クラス0・1のクラス別e-SRを追加する。3成分へ各`1/3`の
固定重みを割り当てた混合e値で検知するため、新しい重みパラメータは設けない。クラス成分の寄与が
最大なら、そのクラス系列の候補開始位置を全体ストリーム位置へ戻してFIFOを分割する。
通信・サーバ処理はv2/v2.1/v2.2と同じである。

v2.2/v2.3およびv3.2/v3.3の`*_ucb`モードは、検知区間開始時の基準平均計算だけを
...
`FedSDA_v3.1`は、このv3のシーケンスと通信内容を変更せず、クライアントのADWIN更新だけを
v2.1と同じ「全体損失＋正解クラス別損失」の並列監視へ置き換える。クラス別検知時の
FIFO分割点復元、全検知器のリセット、共通`ADWIN_DELTA`の扱いもv2.1と同一である。

`FedSDA_v3.2`もv3の通信内容を変更せず、クライアント検知だけをv2.2と同じe-SRへ置き換える。
基準平均、ARLとしてのα、強制チェック無効化の扱いもv2.2と同一である。

`FedSDA_v3.3`はv3の通信内容を維持し、クライアント検知だけをv2.3と同じ全体・クラス別e-SR
混合へ置き換える。クラス別候補位置の復元方法もv2.3と同一である。
