...
同じ6条件で誤り駆動probeを追加すると、直前版よりaccuracyは6条件すべてで改善し、平均+0.086ポイントを
回復した。active集合なしとの平均差はaccuracy -0.143ポイント、stable accuracy -0.043ポイントであり、
推論例数はデータセット別に約17～19%少なかった。真のドリフト後0～10標本の差は-0.94から-0.47ポイント、
10～30標本は-4.41から-3.18ポイント、30～100標本は-0.88から-0.40ポイントへ縮小したため、再probeは
意図した回復遅延を実際に緩和している。一方、平均probe率は50.5%から54.6%へ増え、1 runあたりの誤り駆動
probeはCircle2で約345回、MNIST2で約988回、MNIST4で約1078回発生しても、active集合なしを上回る
accuracyには至らなかった。

したがって、実active集合は精度優先の既定方式にはしない。ただしactive集合なしよりforward計算量が少なく、
直前版より精度が高いため、計算量制約下のPareto方式としては支配されていない。これ以上、同じ周期probeへ
条件を追加する細かな改良は止める。再検討する場合は、部分情報向けオンライン学習や通信時のversioned cacheなど、
周期的な実予測除外とは異なる設計として切り分ける。
