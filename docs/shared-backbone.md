...
最上位Meta-switchingまで集約ごとに再始動する方式も検証したが、4データセット・3 seed・
集約間隔`50/200`の全体でaccuracyが低下した。再始動直後にSwitching候補の採用率が大幅に下がり、
集約前から有効だった追従証拠を失う影響が、候補更新による証拠陳腐化の解消を上回ったためである。
この方式は選択肢として残さず、最上位ルータは集約をまたいで証拠を維持する。
...
共有部・概念別部を完全一致する既知版から再利用し、結果を変えずに上り・下り通信を省く
versioned cacheを検討した。Circle2・MNIST2・MNIST4の下り予備実験では12 runすべてで
削減率が0だった。さらに全6データセット、2 seed、集約間隔50・500の双方向実験では、
上り削減率0.606%、下り削減率はほぼ0%、総パラメータ通信削減率0.289%に留まった。
共有部は局所学習とFedAvgでほぼ毎回変化し、概念別部も再利用可能な期間が短いためである。
版管理・差分manifestの複雑さに見合わないため実装とオプションは削除し、結果不変な通信最適化の
負の結果として記録する。
