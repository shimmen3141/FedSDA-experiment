...
flowchart LR
  ...
  subgraph group_clustering[clustering]
    ...
    clustering_consolidation["<b>クラスタリング後処理</b><br/>merge | representative_merge | parameter_share | noninferiority_merge"]
  ...
  server_flow -.->|"representative_merge, parameter_share, noninferiority_merge: NoCachedが必要"| clustering_consolidation
...
| `clustering_consolidation` | cli: `--clustering-consolidation` | 実装済み | 対象外 | 対象外 | 対象外 | クラスタ決定後に加重平均・既存代表・パラメータ共有・非劣性検証のいずれを行うか |
