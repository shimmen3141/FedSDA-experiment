...
flowchart LR
  ...
  subgraph group_model[model]
    ...
    shared_backbone_gradient_strategy["<b>共有勾配統合</b><br/>mean | pcgrad | heldout_selected"]
...
| `shared_backbone_gradient_strategy` | cli: `--shared-backbone-gradient-strategy` | 実装済み | 理論上のみ | 対象外 | 対象外 | 共同学習時の概念別バックボーン勾配を平均、競合射影、または独立した直近標本による候補選択で統合する方式 |
